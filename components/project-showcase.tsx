...
export function ProjectShowcase({ project, content, index, viewer }: { project: any; content: any; index: number; viewer: any }) {
  const isDesktopVisual = project.slug === "delni-web" || project.slug === "nexus-pos";
  const images = isDesktopVisual
    ? [project.heroImage, ...project.gallery].map((image: any) => ({ src: image.src, alt: content.screenshotAlts?.[image.altKey] ?? content.imageAlt }))
  ...
  return <article className={`project-showcase reveal ${index % 2 ? "reverse" : ""}`}>
    <ProjectImageLightbox images={images} variant={project.slug === "delni-app" ? "delni" : isDesktopVisual ? "website" : "almondas"} index={index} galleryLabel={content.galleryLabel} labels={viewer} />
